...
import me.topilov.springplayground.protection.infrastructure.store.RedisCooldownStore
...
class RedisCooldownStoreTest {
    ...
    fun `activate if absent returns remaining ttl when cooldown already exists`() {
        val first = store.activateIfAbsent("protection:cooldown:key", Duration.ofMinutes(1))
        val second = store.activateIfAbsent("protection:cooldown:key", Duration.ofMinutes(1))
